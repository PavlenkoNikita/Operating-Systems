...
#endif
	STYLE_(HDS_NOSIZING),           0,  -1, 0,          //0x0800
	- 1, _T(""), -1, -1, -1
};
...
void StyleWindow::FillStyleExLists(BOOL fAllStyles, BOOL fExtControl)
{
	_exStyles.clear();
	DWORD dwMessage;
	StyleLookupEx *StyleList;

	DWORD dwStyleEx = (DWORD)GetWindowLongPtr(_handle, GWL_EXSTYLE);

	EnumStyles(_exStyles, StyleExList, dwStyleEx, fAllStyles);

	if (fExtControl)
	{
		StyleList = FindStyleList(ExtendedControls, &dwMessage);

		if (StyleList != 0)
		{
			dwStyleEx = (DWORD)SendMessage(_handle, dwMessage, 0, 0);
			EnumStyles(_exStyles, StyleList, dwStyleEx, fAllStyles);
		}
	}
}

void StyleWindow::FillStyleLists(BOOL fAllStyles, BOOL fExtControl)
{
	_styles.clear();
	DWORD dwMessage;
	StyleLookupEx *StyleList;

	DWORD dwStyle = (DWORD)GetWindowLongPtr(_handle, GWL_STYLE);
	
	dwStyle = EnumStyles(_styles, WindowStyles, dwStyle, fAllStyles);

	StyleList = FindStyleList(StandardControls, 0);
	if (StyleList != 0)
		dwStyle = EnumStyles(_styles, StyleList, dwStyle, fAllStyles);

	StyleList = FindStyleList(CustomControls, 0);
	if (StyleList != 0)
		dwStyle = EnumStyles(_styles, StyleList, dwStyle, fAllStyles);
	...
	if (dwStyle != 0)
	{
		...
		sprintf_s(ach, szHexFmt, dwStyle);
		_styles.push_back(ach);
	}
}

DWORD StyleWindow::EnumStyles(std::vector<std::basic_string<TCHAR>>& styles, StyleLookupEx * StyleList, DWORD dwStyle, BOOL fAllStyles)
{
...
StyleLookupEx * StyleWindow::FindStyleList(ClassStyleLookup * pClassList, DWORD * pdwData)
{
	int i;
	for (i = 0; pClassList[i].stylelist != 0; i++)
	{
		if (lstrcmpi(_className, pClassList[i].szClassName) == 0)
		{
...
StyleWindow::StyleWindow(HWND handle)
	: _handle(handle)
{
	GetClassName(_handle, _className, MAX_PATH);
	FillStyleLists(FALSE,
		TRUE);
	FillStyleExLists(FALSE,
		TRUE);
}

std::vector<std::basic_string<TCHAR>> StyleWindow::GetStyles()
{
	return _styles;
}

std::vector<std::basic_string<TCHAR>> StyleWindow::GetStylesEx()
{
	return _exStyles;
}